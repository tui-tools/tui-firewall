// Package tuifirewall exists for one reason: to embed the repository's
...
// version of the backend it is about to drive. go:embed cannot reach outside
// its own package directory, so the embedding package is the module root.
package tuifirewall
